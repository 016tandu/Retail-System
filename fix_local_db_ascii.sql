...
-- 2. Create the automated function (ASCII ONLY)
...
BEGIN
    ...
    IF v_provider_nv IS NOT NULL AND v_retailer_nv IS NOT NULL THEN
        ...
        INSERT INTO public."STOCK_TRANSFER" (
            ...
            'AUTO-TRANSFER: ' || v_provider_nv."MaNV" || ' to ' || v_retailer_nv."MaNV" || ' at ' || to_char(now(), 'HH24:MI')
        );
    END IF;
    ...
    IF v_staff_nv IS NOT NULL AND v_ma_sp IS NOT NULL THEN
        ...
        VALUES (v_ma_hd, now(), v_staff_nv."MaNV", v_staff_nv."MaKho", v_gia, 'AUTO-INVOICE by ' || v_staff_nv."MaNV" || ' at ' || to_char(now(), 'HH24:MI'));
...
-- 4. Evidence
SELECT 'CONCRETE EVIDENCE - LATEST INVOICES:' as report_title;
