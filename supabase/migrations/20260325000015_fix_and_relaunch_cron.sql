-- 1. Hủy lịch các Job cũ một cách an toàn (Chỉ xóa nếu tồn tại)
DO $$
BEGIN
...
-- 2. Đảm bảo hàm run_automated_business_flow chính xác
...
BEGIN
    ...
    -- A. CHUYỂN KHO (PROVIDER -> RETAILER)
    IF v_provider_nv IS NOT NULL AND v_retailer_nv IS NOT NULL THEN
        ...
        INSERT INTO public."STOCK_TRANSFER" (
            ...
            format('Hệ thống tự động chuyển [%s]. Gửi: %s (%s). Nhận: %s (%s)', 
                   to_char(now(), 'DD/MM HH:MI'), v_provider_nv."HoTen", v_provider_nv."MaNV", v_retailer_nv."HoTen", v_retailer_nv."MaNV")
        );
    END IF;
    ...
    IF v_staff_nv IS NOT NULL THEN
        ...
        v_ma_hd := 'HD-AUTO-' || to_char(now(), 'YYMMDD') || '-' || upper(substring(md5(random()::text) from 1 for 4));
        ...
        VALUES (v_ma_hd, now(), v_staff_nv."MaNV", v_staff_nv."MaKho", v_gia, 
               format('Hóa đơn [%s] bởi %s (%s)', to_char(now(), 'DD/MM HH:MI'), v_staff_nv."HoTen", v_staff_nv."MaNV"));
